...
public abstract class ChallengeMenuGenerator extends MultiPageMenuGenerator {
  ...
  public ChallengeMenuGenerator(int startPage) {
    this(startPage, player -> Challenges.getInstance().getMenuManager().openMainMenuInstantly(player));
  }
  ...
  public MenuPosition createMenuPosition(int page) {
    return new ChallengeMenuPositionLegacyGenerator(this, page);
  }
  ...
  public void updateItem(IChallenge challenge) {
    ...
    if (newSuffix && ChallengeAnnotations.isNew(challenge)) {
      inventory.setItem(slot + 1, new LegacyItemBuilder(Material.LIME_STAINED_GLASS_PANE, "§0").build());
      inventory.setItem(slot + 28, new LegacyItemBuilder(Material.LIME_STAINED_GLASS_PANE, "§0").build());
    } else if (updatedSuffix && ChallengeAnnotations.isUpdated(challenge)) {
      inventory.setItem(slot + 1, new LegacyItemBuilder(Material.YELLOW_STAINED_GLASS_PANE, "§0").build());
      inventory.setItem(slot + 28, new LegacyItemBuilder(Material.YELLOW_STAINED_GLASS_PANE, "§0").build());
    }
  }
  ...
  protected LegacyItemBuilder getDisplayItemBuilder(@NotNull IChallenge challenge) {
    try {
      LegacyItemBuilder item = new LegacyItemBuilder(challenge.getDisplayItem(null).build()).hideAttributes();
      if (newSuffix && ChallengeAnnotations.isNew(challenge)) {
  ...
  private class ChallengeMenuPositionLegacyGenerator extends LegacyGeneratorMenuPosition {

    public ChallengeMenuPositionLegacyGenerator(MenuGenerator generator, int page) {
