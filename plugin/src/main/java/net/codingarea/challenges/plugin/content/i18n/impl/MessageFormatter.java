...
import static net.codingarea.challenges.platform.message.MessagePlatform.*;
...
public final class MessageFormatter { // pre expansion for better server runtime performance
  ...
  public static boolean isMetaTag(@NotNull String keyName) {
    ...
    return relativeKeyName.equals(COLORIZE_KEY); // currently only meta implemented
  }
  ...
  private static String[] embedReferences(@NotNull String[] origin, @NotNull String originKey,
    ...
    colorize(origin, originSubDocumentPath, messagesBundle); // apply colorization if defined for this sub-document
    ...
    for (String line : origin) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (c == REFERENCE_ARG_CHAR && inReferenceArguments) {
          if (inReferenceArgument) {
            ...
            referenceArguments.add(builtReferenceArgument);
      ...
      if (!reference.isEmpty()) { // arg was not closed
        System.err.println("Unclosed reference in '" + line + "' of " + originKey + ": '" + reference + "'");
        builder.append(START_ARG_CHAR).append(reference);
      }
      if (!referenceArgument.isEmpty()) { // arg was not closed
        System.err.println("Unclosed reference argument in '" + line + "' of " + originKey + ": '" + referenceArgument + "'");
  ...
  private static void resolveReferenceInto(@NotNull String originKey, @NotNull Map<String, String[]> messagesBundle,
    ...
    if (rawReferenceValue != null && rawReferenceValue.length != 0) {
      ...
      System.out.println("Resolved reference '" + relativeReferenceName + "' (qualified: " + qualifiedReferenceName + ") in " + originKey +
        ": " + Arrays.toString(referenced) + " -> " + Arrays.toString(embeddedReferenced));
  ...
  private static String qualifyRelativeKey(@NotNull String relativeReferenceName, @NotNull String[] originSubDocumentPath,
                                           @NonNull Map<String, String[]> messagesBundle) {
  ...
  private static void colorize(@NotNull String[] rawValue, @NotNull String[] originSubDocumentPath, @NotNull Map<String, String[]> messageBundle) {
    ...
    StringBuilder builder = new StringBuilder(rawValue[0].length() + colorizeValue[0].length());
    StringBuilder argument = new StringBuilder(32);
    boolean colorizing = false;
    for (int line = 0; line < rawValue.length; line++) {
      String text = rawValue[line];
      int len = text.length();
      for (int i = 0; i < len; i++) {
      ...
      rawValue[line] = builder.toString();
      builder.setLength(0); // reset line for next iteration
    }
  }
